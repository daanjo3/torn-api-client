...
export default class TornClient {
    ...
    async execute(url: string): Promise<any> {
        ...
        if (response.data['error'] != null) {
            if (this.throwError) {
                throw new Error(`Request failed with error: ${response.data['error']['error']}`)
            }
        }
    ...
    buildUrl(compartment: string, selections: string[], id?: string): string {
        ...
        return `${this.baseUrl}/${compartment}/${idParam}` + 
     ...
     */
    verifySelections(selections: string[], isSelection: (value: string) => boolean): void {
